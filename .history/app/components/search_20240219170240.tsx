...
const Search = (props: Props) => {
    ...
    const url:Props['url'] = 'https://work-out-api1.p.rapidapi.com/search?Muscles=biceps';
    ...
    async function handleSearch(term: string) {
       const params = new URLSearchParams(searchParams);
        ...
        if (term){
            const data = await FetchData(url, options: props.options).then((res)=> res.json());  
             return <div className="flex flex-col w-full">{data?.exercises.map(e=><Exercise key={e.Id} exercise={
